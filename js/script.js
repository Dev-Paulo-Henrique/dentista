(function () {
  ...
  var startMessage = new ObjectMessage(
    cnv.height / 2,
    "PRESSIONE ENTER",
    "#fff"
  );
  ...
  function loop() {
    ...
    switch (gameState) {
      ...
      case OVER:
        ...
        endGame();
        setTimeout(() => {
          location.replace("https://dev-paulo-henrique.github.io/dentista/");
        }, 3000);
        break;
    }
    render();
  ...
  function update() {
    ...
    defender.x = Math.max(
      0,
      Math.min(cnv.width - defender.width, defender.x + defender.vx)
    );
  ...
  function updateScore() {
    ...
    $("#record-pontos")
      .css("color", "#0f0")
      .text(localStorage.getItem("Pontos"));
    $("#record-taxa")
      .css("color", "#0f0")
      .text(localStorage.getItem("Taxa"))
      .append("%");
    $("#record-tentativas")
      .css("color", "#0f0")
      .text(localStorage.getItem("Tentativas"));
